...
class Chunk(object):
  ...
  def get(self, index):
    ...
    if index < 0 or index > len(self._records):
      raise RuntimeError('illegal index value for the records size is ' + str(len(self._records)))
  ...
  def write(self, out_file, compressor):
    ...
    header = Header(self._total_count, checksum, compressor, len(compressed_data))      
  ...
  def parse(self, in_file, offset):
    ...
    if offset < 0 or offset >= (file_size - int_word_len - 1):
      raise RuntimeError('invalid offset ' + str(offset) + ' total file size ' + file_size)
    ...
    if real_checksum != raw_checksum:
      raise RuntimeError("checksum check failed for raw checksum " + str(raw_checksum) + " and new checksum " + str(real_checksum))
    ...
    while record_count < header.total_count():
      rc_len = int.from_bytes(uncompressed_byte_arr[curr_index:curr_index+int_word_len], endian)               
