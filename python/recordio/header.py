...
class Header(object):
  ...
  def __init__(self, num_records=0, checksum=None, compressor=None, compress_size=0):
    # Magic number.
  ...
  def parse(self, in_file, offset):
    ...
    if offset < 0 or offset >= (file_size - int_word_len - 1):
      raise RuntimeError('invalid offset ' + str(offset) + ' total file size ' + str(file_size))
    ...
    self._compressor = Compressor(int.from_bytes(in_file.read(int_word_len), endian))
    self._compress_size = int.from_bytes(in_file.read(int_word_len), endian)
...
class Compressor(Enum):
  ...
  no_compression = 1
  # Compression algorithm from google which aims for very high speeds and reasonable compression.
  snappy = 2
