...
[#675](https://github.com/sql-machine-learning/sqlflow/issues/674) for details.
* Switch to use intermediate representation for ElasticDL codegen. For details,
please see [#1075](https://github.com/sql-machine-learning/sqlflow/issues/1075).
...
ElasticDL job for a model defined in model zoo.
Please see [#22](https://github.com/sql-machine-learning/models/issues/22) and
...
expose necessary abstractions so ElasticDL
...
* Currently we have hard-coded the types for each column to be ``tf.float32``
in the generated `dataset_fn`. We should infer
