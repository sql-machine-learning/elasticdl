...
class UpdateVariableBySizeHook(tf.train.SessionRunHook):
    def __init__(self, updatable_tensor, total_count):
        self._step = 0
        self._value_placeholder = tf.placeholder(updatable_tensor.dtype, [])
        self._update_op = tf.assign(updatable_tensor, self._value_placeholder)
    ...
    def after_run(self, run_context, run_values):
        size = hvd.size()
        rank = hvd.rank()
        this_worker_count = self._total_count // size + int(rank < self._total_count % size)
        run_context.session.run(self._update_op, feed_dict={self._value_placeholder: this_worker_count})
        if self._step % 10 == 0:
            print("****** Set the variable to be {} ******".format(this_worker_count))
...
## `ElasticDistributedOptimizer` is slightly updated based on https://github.com/horovod/horovod/blob/18cfedd5236885d9fd581ca53fe04a142486bb2b/horovod/tensorflow/__init__.py#L294
class ElasticDistributedOptimizer(tf.train.Optimizer):
    ...
    def __init__(self, optimizer, name=None, use_locking=False, device_dense='',
        ...
        super(ElasticDistributedOptimizer, self).__init__(name=name, use_locking=use_locking)
