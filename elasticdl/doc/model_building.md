...
- records: a list of training data generated by ElasticDL. ElasticDL reads and decodes a batch of data from RecordIO file.
...
`feature_columns` is also used for decoding the training data in RecordIO file. This requires that [`input_fn`]($input_fn) cannot change the training data type and shape in data processing. This limitation will be addressed later in ElasticDL development.
...
`label_columns` returns a list of [`tf.feature_column.numeric_column`](https://www.tensorflow.org/api_docs/python/tf/feature_column/numeric_column), which is used for decoding the label data in RecordIO file.
