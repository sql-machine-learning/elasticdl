...
func (opt *AdamOptimizer) InitNonEmbeddingParam(name string, dim []int64) {
	opt.m.NonEmbeddingParam[name] = common.NewEmptyTensor(name, dim, opt.Dtype)
	opt.v.NonEmbeddingParam[name] = common.NewEmptyTensor(name, dim, opt.Dtype)
	opt.maxSquare.NonEmbeddingParam[name] = common.NewEmptyTensor(name, dim, opt.Dtype)
}
