...
func TestSGDOptimizer(t *testing.T) {
	...
	t1 := commonnew.NewTensor(v1, d1) //t1

	d2 := []int64{2, 2}
	v2 := []float32{1.0, 2.0, 1.1, 2.2}
	t2 := commonnew.NewTensor(v2, d2) //t2
	...
	grad1 := commonnew.NewTensor(gv1, d1) //t1
	grad2 := commonnew.NewTensor(gv2, d2) //t2
	pbModel := &proto.Model{
	...
	assert.True(t, commonnew.CompareFloatArray(commonnew.Slice(model.DenseParameters["t1"]).([]float32), ev1, 0.0001))
	assert.True(t, commonnew.CompareFloatArray(commonnew.Slice(model.DenseParameters["t2"]).([]float32), ev2, 0.0001))

	// test grad name error
	grad3 := commonnew.NewTensor(gv2, d2) //t3
	pbModel = &proto.Model{
	...
	i3 := []int64{1, 3}
	grad3 = commonnew.NewTensor(v3, d3) // t3
	sgrad3 := commonnew.NewIndexedSlices(grad3, i3)
	...
	assert.True(t, commonnew.CompareFloatArray(expV, commonnew.Slice(vectors).([]float32), 0.0001))
