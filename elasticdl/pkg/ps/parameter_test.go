...
func TestParameterInitFrom(t *testing.T) {
	...
	assert.True(t, common.CompareFloatArray([]float32{1.0, 2.0}, ev1.Value, 0.0001))

	ev3 := e1.GetEmbeddingVector(3)
	assert.True(t, common.CompareFloatArray([]float32{3.0, 4.0}, ev3.Value, 0.0001))

	ev5 := e1.GetEmbeddingVector(5)
	assert.True(t, common.CompareFloatArray([]float32{5.0, 6.0}, ev5.Value, 0.0001))
}
