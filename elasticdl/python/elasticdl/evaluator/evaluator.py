...
class Evaluator(object):
    ...
    def __init__(
        ...
        Arguments:
            ...
            max_minibatch_retry_num: The maximum number of a minibatch 
                retry as its results(e.g. gradients) are not accepted 
                by master.
        """
    ...
    def run(self):
        ...
        for file_name in os.listdir(self._data_dir):
            ...
            with closing(
                recordio.Scanner(self._data_dir + "/" + file_name)
            ) as reader:
                while True:
                    record_buf = self._get_batch(
                        reader, self._batch_size, self._codec.decode
                    )
                    if not record_buf:
                        break
                    features, labels = self._get_features_and_labels_from_record(
                        record_buf
                    )
                    ...
                    self._logger.info(
                        "Model loss: %f, accuracy: %f" % (loss, accuracy)
                    )
        ...
        self._logger.info(
            "Averaged model loss: %f, accuracy: %f" % (avg_loss, avg_accuracy)
        )
