...
"""
    ...
    def _m_file_in_docker(self, model_file):
        return "/model/" + os.path.basename(model_file)

    def _build_docker_image(
        self, m_file, image_name, push_image, image_base="elasticdl:dev"
    ):
        docker_template = """
    FROM {}
    COPY {} {}
        ...
        with tempfile.NamedTemporaryFile(mode="w+", delete=False) as df:
            df.write(
                docker_template.format(
                    image_base, m_file, self._m_file_in_docker(m_file)
                )
            )
    ...
    def _gen_master_def(self, image_name, model_file, job_name, args, argv):
        master_yaml = """
    apiVersion: v1
    kind: Pod
    metadata:
    ...
    spec:
      ...
      restartPolicy: Never
    """.format(
            m_file=self._m_file_in_docker(model_file),
    ...
    def _submit(self, image_name, model_file, job_name, args, argv):
        master_def = self._gen_master_def(
            image_name, model_file, job_name, args, argv
        )
