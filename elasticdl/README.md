...
```bash
make && K8S_TESTS=False python -m unittest discover elasticdl/python/tests/ '*_test.py'
```
...
docker run --rm -u $(id -u):$(id -g) -it \
    ...
    -w /v \
    elasticdl:dev \
    bash -c "make && K8S_TESTS=False python -m unittest discover elasticdl/python/tests/ '*_test.py'"
```
